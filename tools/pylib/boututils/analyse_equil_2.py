...
def analyse_equil(F, R, Z):
    ...
    w = numpy.where(
        (rex1 > R[2]) & (rex1 < R[nx - 3]) & (zex1 > Z[2]) & (zex1 < Z[nx - 3])
    )
    ...
    mind = (opt_ri[0] - (old_div(numpy.float(nx), 2.0))) ** 2 + (
        opt_zi[0] - (old_div(numpy.float(ny), 2.0))
    ) ** 2
    ind = 0
    for i in range(1, n_opoint):
        d = (opt_ri[i] - (old_div(numpy.float(nx), 2.0))) ** 2 + (
            opt_zi[i] - (old_div(numpy.float(ny), 2.0))
        ) ** 2
        if d < mind:
            ind = i
            mind = d
    ...
    if n_xpoint > 0:
        ...
        for i in range(n_xpoint):
            ...
            for j in range(n):
                ...
                farr[j] = func(
                    opt_ri[ind] + dr * numpy.float(j), opt_zi[ind] + dz * numpy.float(j)
                )
