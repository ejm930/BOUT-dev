...
def showdata(
    ...
    for i in range(0, Nvar):
        ...
        if not (global_colors):
            if isclose(fmin[i], fmax[i]):
                ...
                thiscontourmin = fmin[i] - 3.0e-15 * abs(fmin[i]) - 1.0e-36
                thiscontourmax = fmax[i] + 3.0e-15 * abs(fmax[i]) + 1.0e-36
    ...
    if global_colors:
        ...
        if isclose(fminglobal, fmaxglobal):
            fminglobal = fminglobal - 3.0e-15 * abs(fminglobal) - 1.0e-36
            fmaxglobal = fmaxglobal + 3.0e-15 * abs(fmaxglobal) + 1.0e-36
        for i in range(0, Nvar):
    ...
    for i in range(0, Nvar):
        ...
        if lineplot[i] == 1:
            ...
            plots.append(
                ax[i].contourf(
                    theta[i], r[i], vars[i][0][0, :, :].T, cmap=cmap, levels=clevels[i]
                )
            )
            plt.axes(ax[i])
            cbars.append(fig.colorbar(plots[i], format="%1.1e"))
