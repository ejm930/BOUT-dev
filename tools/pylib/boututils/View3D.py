...
if sys.version_info[0] >= 3:
    message = (
        "View3D uses the VTK library through mayavi, which"
...
def View3D(g, path=None, gb=None):
    ...
    xf = (
        np.arange(np.float(g.qpsi.size))
        * (g.sibdry - g.simagx)
        / np.float(g.qpsi.size - 1)
        + g.simagx
    )
    ...
    B1p, B2p, B3p, B1t, B2t, B3t = magnetic_field(
    ...
    iso = mlab.pipeline.iso_surface(
        field2, contours=[Psi.min() + 0.01], opacity=0.4, colormap="bone"
    )
...
def magnetic_field(g, X, Y, Z, rmin, rmax, zmin, zmax, Br, Bz, Btrz):
    ...
    for i in range(len(k)):
        br[k[i, 0], k[i, 1], k[i, 2]] = fr(
            rho[k[i, 0], k[i, 1], k[i, 2]], Z[k[i, 0], k[i, 1], k[i, 2]]
        )
        bz[k[i, 0], k[i, 1], k[i, 2]] = fz(
            rho[k[i, 0], k[i, 1], k[i, 2]], Z[k[i, 0], k[i, 1], k[i, 2]]
        )
        bt[k[i, 0], k[i, 1], k[i, 2]] = ft(
            rho[k[i, 0], k[i, 1], k[i, 2]], Z[k[i, 0], k[i, 1], k[i, 2]]
        )
