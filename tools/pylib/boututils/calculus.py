...
def deriv(*args, **kwargs):
    ...
    if periodic:
        ...
        if n > 2:
            ...
            result[0] = old_div(
                (-1.5 * var[0] + 2.0 * var[1] - 0.5 * var[2]), (x[1] - x[0])
            )
            result[n - 1] = old_div(
                (1.5 * var[n - 1] - 2.0 * var[n - 2] + 0.5 * var[n - 3]),
                (x[n - 1] - x[n - 2]),
            )
        elif n == 2:
...
def deriv2D(data, axis=-1, dx=1.0, noise_suppression=True):
    ...
    if axis == 1:
        ...
        for i in range(s[1]):
            ...
            data_deriv[s[0] - N : s[0] + 1, i, 0] = old_div(
                deriv(data[s[0] - N : s[0] + 1, i]), dx[0]
            )
        ...
        for i in range(s[0]):
            ...
            data_deriv[i, s[1] - N : s[1] + 1, 1] = old_div(
                deriv(data[i, s[1] - N : s[1] + 1]), dx[1]
            )
    ...
    if periodic:
        ...
        def int_total(f):
            ...
            if n > 7:
                ...
                return 3.0 * (f[0] + 3.0 * f[1] + 3.0 * f[2] + f[3]) / 8.0
            elif n == 3:
                # 4th-order Simpson's rule
                return (f[0] + 4.0 * f[1] + f[2]) / 3.0
            elif n == 2:
