    ...
    for x in range(nx):
        ...
        for y in range(ny - 1):
            ...
            for i in range(nskip[y]):
                ...
                f[ypos + i] = w * zinterp(vr[x, y + 1, :], zi) + (1.0 - w) * zinterp(
                    vr[x, y, :], zi
                )
                ...
                BtBp[ypos + i] = old_div(
                    (w * Btxy[x, y + 1] + (1.0 - w) * Btxy[x, y]),
                    (w * Bpxy[x, y + 1] + (1.0 - w) * Bpxy[x, y]),
                )
        ...
        if numpy.max(theta) > 1.0:
            ...
            if quiet is None:
                print("Mismatch  ", x, numpy.max(theta))
        ...
        tarr = (
            2.0 * numpy.pi * numpy.arange(np) / numpy.float(np)
        )  # regular array in theta
    ...
    if slow is not None:
        ...
        for i in range(nf):
            if numpy.max(famp[:, i]) > 0.05 * numpy.max(famp):
                ...
                plot([pos, pos], [0, 2.0 * numpy.max(fmax)], "k--")
                draw()
    ...
    #        ENDIF
        ...
        if subset is not None:
            ...
            for i in range(minind, maxind + 1, subset):
                ...
                plot([pos, pos], [0, 2.0 * numpy.max(fmax)], "--")
